...
pattern_detener = re.compile(r"\b(para|basta|detente|termina|atras|retroceder)\b", re.IGNORECASE)
...
def hora():
    ...
    label_info.config(text=f"Hora actual: {hora_actual}")
    ajustar_tamaño_texto()
...
def esperar_alarma(tiempo_alarma):
    ...
    zone_label.config(text=tiempo_alarma.strftime("%H:%M"))
    zone_label.update()
    ...
    while alarma_activada:
        ...
        if pattern_detener.search(parar):
            zone_label.config(text="Alarma detenida")
            ajustar_tamaño_texto()
            speak("Alarma detenida.")
            alarma_activada = False
            activacion = False
            zone_label.config(text="")
...
def esperar_temporizador(duracion_segundos):
    ...
    while tiempo_restante > 0:
        ...
        zone_label.config(text=tiempo_formateado)
        zone_label.update()
    ...
    # Actualizar la etiqueta al finalizar el temporizador
    zone_label.config(text="¡Temporizador finalizado!")
    while alarma_activada:
        speak("¡Alarma!")
        time.sleep(0.5)
        speak("¡Alarma!")
...
def terapia():
    while True:
        ...
        terapia = None
        while not terapia:
            speak("Por favor, escoge entre jugar y recuerdos.")
            ...
            if not terapia:
                speak("No se pudo entender la eleccion. Por favor, intenta de nuevo.")
...
def update_advice():
    ...
    advice_label.config(text=random_advice)
    root.after(5000, update_advice)

# Lista de consejos
advice_list = [
    "Consejo 1: Mantén la calma y programa con Python.",
    "Consejo 2: Aprende algo nuevo cada día.",
    "Consejo 3: No temas cometer errores, son parte del aprendizaje.",
    "Consejo 4: Mantén un equilibrio entre trabajo y descanso.",
    "Consejo 5: Si te sientes atascado, pide ayuda a la comunidad."
]
...
# Configuración del fondo que ocupa toda la pantalla
...
advice_label = tk.Label(label_info, text="", font=("Helvetica", 20))
advice_label.place(relx=0.50, rely=0.90)
update_advice()

# Configuración del reloj en la esquina superior izquierda
clock_label = tk.Label(label_info, text="", font=("Helvetica", 20))
clock_label.place(relx=0.05, rely=0.05)
update_clock()

# Configuración de la zona en la parte superior central, poner un contador o una alarma programada
zone_label = tk.Label(label_info, text="", font=("Helvetica", 20))
zone_label.place(relx=0.90, rely=0.05)

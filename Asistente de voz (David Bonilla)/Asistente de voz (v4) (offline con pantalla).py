...
def esperar_temporizador(duracion_segundos):
    ...
    label_info.config(text="¡Temporizador finalizado!")
    ajustar_tamaño_texto()
        ...
        if not alarma_activada:
            ...
            if query and activacion:                
                ...
                # Procesamiento de consultas generales
                    ...
                    label_info.config(text="Lo siento, no entendí eso. ¿Puedes repetir?")
                    ajustar_tamaño_texto()
...
def calcular_letras_por_linea(ventana):
    ...
    for tamaño_fuente in range(10, 51):  # Tamaños de fuente de 10 a 50
...
def calcular_lineas_por_pantalla(ventana):
    ...
    for tamaño_fuente in range(10, 51):  # Tamaños de fuente de 10 a 50
        font = Font(size=tamaño_fuente)
        altura_linea = font.metrics("linespace")
...
label_info = tk.Label(root, text="Lopako", font=("Arial", 50), justify="center", anchor="center", wraplength=root.winfo_width() - 20)
label_info.pack(expand=True, fill='both')

letras_por_linea = calcular_letras_por_linea(root)
lineas_por_pantalla = calcular_lineas_por_pantalla(root)
